...
* Надежность и предсказуемость работы системы.
* Легкость в обслуживании и поддержке.

***Минусы:***
* Отсутствие асинхронных вызовов и реактивного взаимодействия.
* Невозможность самостоятельного подключения датчиков пользователями.
* Ограниченная текущая функциональность (только управление отоплением и проверка температуры) и сложность расширения функциональности.
* Ограниченные возможности по горизонтальному масштабированию в случае повышения нагрузки.
...
Целевая (TO-BE) архитектура решения Smart Home построена с использованием микросервисного подхода. 

Технологический стек решения остается без изменений.

В соответствии с результатами DDD для каждого основного домена выделены отдельные микросервисы. Также при проектировании архитектуры приложения в части работы с датчиками применен подход CQRS, разделяющий поток получения информации с датчиков и сохранения их БД и поток чтения информации полученной с датчиков пользователями. Таким образом выделены следующие основные микросервисы:


| Наименование       | Описание                                                                                                                                                  |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API gateway        | Основной шлюз, обеспечивающий централизованный доступ к API Smart House и решающий задачи аутентификациии и авторизации                                   |
| Profile management | Микросервис, обеспечивающий регистрацию пользователей и домов и предоставляющий методы по работе с профилем пользователя                                  |
| Device management  | Микросервис, обеспечивающий регистрацию устройств и работу с ними                                                                                         |
| Telemetry          | Микросервис, обеспечивающий чтение телеметрии с устройств и предоставления данных телеметрии пользователям. Микросервис реализован согласно паттерну CQRS |

**MVP Containers diagram**
![](./C4-container_MVP.png)

**TO-BE Containers diagram**
![](./C4-container.png)
...
## 2.1 Разработка MVP. Новые микросервисы и интеграция с монолитом
